  ...
  const days = data.days
    .map((day) => {
      ...
      const dayJournals = journals
        ...
        .join('')
      return `<section class="day"><header><small>第 ${day.day} 天 · ${escapeHtml(day.date)} ${escapeHtml(day.weekday)}</small><h2>${escapeHtml(day.region)}</h2><p>${escapeHtml(day.route)}</p></header><h3>文字行程</h3><p class="pre">${escapeHtml(day.schedule)}</p><h3>住宿与其他信息</h3><p class="pre">${escapeHtml(day.lodging)}</p>${places || '<p>本日没有澳洲地图点位。</p>'}${dayJournals ? `<h3>随手记</h3>${dayJournals}` : ''}</section>`
    })
    .join('')
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(data.trip.name)} · 离线路书</title><style>:root{color-scheme:light;--bg:#fbf4df;--paper:#fffdf7;--text:#382b1f;--muted:#766554;--accent:#b85c22;--line:#e8d8bd}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:15px/1.65 -apple-system,BlinkMacSystemFont,"PingFang SC",sans-serif}main{width:min(100% - 28px,760px);margin:auto;padding:28px 0 60px}h1,h2,h3,h4,p{margin-top:0}h1{font-size:32px;line-height:1.1}.notice{padding:12px;border-left:4px solid var(--accent);background:#fff3cd}.day{margin-top:28px;padding:20px;background:var(--paper);border:1px solid var(--line)}.day>header{border-bottom:2px solid var(--accent);margin-bottom:18px}.place{padding:16px 0;border-top:1px solid var(--line)}.place h3{display:flex;gap:8px;align-items:center}.place h3 span{display:grid;width:28px;height:28px;place-items:center;background:var(--accent);color:white;border-radius:50%}.place em{font-size:12px;color:var(--accent)}a{color:var(--accent)}.pre{white-space:pre-line}.journal{padding:12px 0;border-top:1px dashed var(--line)}.photos{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.photos img{width:100%;aspect-ratio:1;object-fit:cover}@media print{body{background:white}.day{break-inside:avoid}}</style></head><body><main><h1>${escapeHtml(data.trip.name)}</h1><p>${escapeHtml(data.trip.dates)}</p><p class="notice">这是可离线打开的静态副本。地图瓦片、实时天气与外部网站仍需联网；${journals.length ? '已按你的选择包含本机随手记。' : '默认未包含本机随手记。'}</p>${days}</main></body></html>`
}
