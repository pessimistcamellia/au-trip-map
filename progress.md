...
- 用户问这个包是否按最新的「新的评审包skill」做的。核对结论：不是，做包时用的是旧 `review-package`（MinIO 通道）。两个 skill 的 `SKILL.md` 差 72 行、`template.html` 差 87 行，逐行看差异全部落在发布与浏览器写回机制上，内容规范（页签顺序、开发团队列、是否必填＋报错反馈文案、弹窗 display 铁律、标注引擎最新、空 td 可填）两边一致，所以内容层不需要返工。
- 顺手查出一个已经发生的故障：旧包内嵌的 MinIO 写回 token 在 9/14 21:57 过期，线上「保存发布 / 同步决策事项」已失效；而 MinIO 网关 presign 会给对象改名，`--mode update` 依赖的页面内 URL 又已过期，原址续签这条路走不通，留在 MinIO 等于评审包只读。据此建议迁移，用户选择评审包与原型一起迁。
- 迁移前先跑 `pull_user_edits.py` 对比线上 MinIO 版与本地产物，输出「内容一致」，确认没有会被抹掉的人工编辑。
- `build.py` 改三处：`TEMPLATE` 指向 `新的评审包skill/template.html`（编辑引擎从 MinIO 预签名 PUT 换成 GitLab Files API），`REVIEW_URL` / `PROTO_URL` 换成 Pages 地址。
- 原型按仓库已有惯例发成 `review-pkgs/<slug>-prototype.html`（不注入 `__SAVE__`，上一个包也是这么发的），评审包用 `publish_gitlab.py --mode first`，`--expires-days 30`（GitLab token 本身有效到 2027-09-10，所以放宽写回窗口，避免再次出现 token 先于评审到期）。
- 中途发现 `build.py` 的 `--sdd-url` / `--sdd-block` 默认是空串，裸跑一次就把原型的 SDD 深链清成空。已把默认值改成线上真源，重建后与已发布副本逐行一致（差异 0 行）。
- 交付门禁 19 项全通过：无 `__SAVE_` 占位符、`__SAVE__.type=gitlab` 且字段真实、exp 有效至 10/15、6 页签且末页为决策事项、校验表两列齐、iframe 指向 Pages 原型、无 MinIO 残留、原型 `syncDialogFollow` 在位、`.modal-mask{display:none}` 默认隐藏、SDD 深链在线。
- 功能实测：headless DOM 里「✎ 编辑」与「同步决策事项」正常渲染、无配置错误红条；GitLab Files API 对 `Origin: https://pages.yc345.tv` 的 PUT 预检返回 200 且允许 `private-token`，写回链路通。发布后 Pages 回读一度 404，是 CI 构建与 CDN 传播延迟，两条流水线 success 后即 200。
- 飞书两份文档按 block 定点替换共 6 处链接（SDD 2 处、需求说明 4 处）。因为多处可见文字都是「打开」，按文字 `str_replace` 会打错行，改用 `docs +fetch --scope keyword` 取块 ID 后 `block_replace`；复核旧 minio 链接残留 0。

## 2026-09-14 无人机选型按纯娱乐与景点拍摄重排
- 用户确认完全自娱自乐、不发布、不变现，且不拍机场附近酒店；移除商业用途和 Belmont 对选型的影响。
- 澄清重量规则：娱乐飞行在 25 kg 以下均无需注册或考试；塔州／维州公园禁令与西澳 DBCA 放行均不按 Mini／Air／Mavic 重量区分。250 g 的主要法规差异只在有塔台机场 5.5 km 内。
- 推荐由 Mini 5 Pro 改为 Air 3S；Mavic 4 Pro、Mavic 3 Pro列为画质优先备选。
- 推荐表补齐同一 DJI 官方商城售价：Air 3S RC 2 畅飞 ¥9,988、Mavic 4 Pro RC 2 畅飞 ¥17,888、Mavic 3 Pro 带屏畅飞 ¥15,919、Mini 5 Pro RC 2 畅飞标价 ¥6,988。
- 本地 Markdown 与原飞书文档已原地覆盖同步，飞书 revision 23，并复核 Air 3S 价格与推荐排名存在。

## 2026-09-14 澳洲租无人机选型
- 官方规格：Mini 5 Pro 249.9 g、1″、4K120、抗风 12 m/s；Air 3S 724 g 双摄；Mavic 4 Pro 约 1063 g 三摄。
- 法规：CASA 娱乐飞无需注册；西澳 DBCA 公园可娱乐飞；塔州 PWS reserved land 一律不许；Parks Victoria 一律不许。City of Belmont（9/27 住宿）超 250 g 禁飞。
- 租赁：国内平台协议未写出境；澳洲当地 Mini 4 Pro 约 ¥3,400–6,100 且不能珀斯取墨尔本还。拿机更优路径是二手 Mini 5 Pro 用完再卖。
- 晨曦Stanley 具体机型未能公开核实，按御系列档位推断。
- 飞书文档已上传生产力方舟：https://guanghe.feishu.cn/docx/GkaNdSJ5movIABx2LdBc8j7Hn8d

## 2026-09-14 中澳无人机租赁与买卖调研
- 核实人人租、品租、御三家、新创 3C 等公开商品页，记录各候选机型可确认的押金、阶梯租金与异常占位价；未把“¥1/天”等引流字段当真实总价。
- 逐字检索人人租、品租官方租赁协议，均未找到“允许出境”或“禁止出境”条款；结论按“必须另取商家书面许可，未取则高风险”处理。
- 核实人人租/品租损坏、丢失、维修期计租、逾期和退押流程；探物仅找到历史用户体验，已明确降级为非官方、非当前条款。
- 核实 Drones For Hire、Perth Camera Hire、Green Sky、Picture Hire、D1、Drone Shop Tasmania 等澳洲渠道；D1 当前暂停，塔州消费级现货和珀斯→墨尔本异地归还均未能核实。
- 核实 CASA 官方娱乐飞行规则和 WA/TAS/VIC 地方规则：WA 公园多可娱乐飞但有野生动物/地点限制；塔州 reserved land 禁止游客放飞；Parks Victoria 禁娱乐无人机；City of Melbourne 公共开放空间须许可且门槛高。
- 核实 DJI 官方新机/翻新价、DJI Care 地域限制和部分二手市场锚点；按 AUD/CNY≈4.81 形成 13 天成本量级。
- 两次澳洲游客租赁条件搜索返回工具错误，已换用具体商家 FAQ/条款、有效 ID 和信用卡 hold 原文补证；未重复失败路径。

## 2026-09-14 澳大利亚无人机法规调研
- 已以 CASA、WA DBCA、Tasmania PWS、Parks Victoria、地方政府、ABF、DJI 与航空公司官方页面核实联邦飞行、公园禁限飞、野生动物、入境与锂电池规则。
- 地点级无法从官方页面确认土地边界或地方禁令的项目均标为「未能核实」，未使用旅游博客替代官方结论。
- 已形成 A/B/C/D 四节中文 Markdown 报告，并对行程地点逐项给出官方 URL 与核实状态。

## 2026-09-09 评审包与原型视觉重做
- 按 redesign audit 逐项改造评审包：暖纸底＋单一孔雀蓝强调色、Instrument Sans/PingFang/Plex Mono 三档字体、去边框表格（仅横线＋等宽编号列）、卡片去边框改染色阴影、非对称两栏、纸面颗粒、hover/active/focus 与 `scroll-behavior`，并补 favicon、description/og 标签与页签 `#tab=` 深链。
- 原型重做为标注式布局：宽屏左侧手机壳（含底部按钮条内置）＋右侧变更批注栏；补齐选项 hover/press、弹窗抓手与滑入动效、`prefers-reduced-motion` 与暗色适配。
- 流程图（`flow.d2`）配色统一到新色板，连线由亮蓝改暖灰，重新出 svg/png。
- 修复批注栏失效：`DIFF_CHANGESET` 改为引擎要求的平铺结构（`page`/`module`/`type`/`level`/`sdd.blockId` + `meta.demoControls`），承载元素改 `id="consumer" data-diff-page="C 端 · 自助换课页"`，`setDemoState(id,value)`、`locate(pageId)` 按契约重写，弹窗恢复 `display:none` 以免锚点错钉。
- 线上实测（新版原型）：批注栏 5 条变更、编号锚点与连线可用、SDD 深链带 block 锚点；勾选→提交→弹窗→确认→成功链路通过；演示状态切「资格拒绝／符合资格」正常；点 C03 卡片会自动打开确认弹窗。
- 发布：评审包原地更新（SHA-256 `bdbf714af4365cad1ed3abbcd68aba9bc179729fb8793088ab3176def20ff6c7`，`validate_minio_release` 通过）；原型因网关不支持原地覆盖改发 `-v2` 键，并把 SDD（1 处）与需求说明（2 处）的原型链接原地改到新地址，复核两文档已无旧链接。

## 2026-09-07 智能客服 AI 自助换课评审包
- 制作并发布可在线编辑的评审包，含简介、变更清单、资格策略、AI golden set、质量门禁、高保真原型和决策事项。
- 制作 C 端标注原型，实测目标选择、两层确认、弹窗关闭、成功结果；线上控制台无业务错误。
- 在生产力方舟同名子文件夹创建需求说明与 SDD，完成双向链接、原生表格和实体归档核验。
- 课程范围作为唯一待定项，给出「规则生成候选 + 业务确认规则与例外 + 无法判断默认拒绝」方案。
- MinIO 评审包 SHA-256：`033fc03a9615042c70ac1d8f10c844f9d8ac4bd5dde613bdc62abe91701c887a`。

## 2026-09-03 纠正 220 cm 认知并列出真 220 清单
- 查实黑冰 L 码为 205 cm（台湾经销商官方规格表逐款佐证）；淘宝「加长型 1.8-2 米」是阿里类目属性字段，非厂商标注。
- 真 220：黑岩暖阳/和风加大 220×85（帽可拆、可拼、鹅鸭可选）、挪客 CW1000 L 220×85；另有三极 230×90、C2H3 215、Helsport Long 等。
- 提示：220 只对男方有价值，女 160 用 220 会多死腔更冷。

## 2026-09-03 迪卡侬睡袋对照
- 中国官网类目以 Quechua Arpenaz 0℃（¥399.9 / 2.2 kg）为主；欧英站核对 MT500/MT900 重量、ISO 温标、是否可拼。
- 结论：对 −10℃ + 轻量 + 鹅绒四档，迪卡侬没有对打款；MT900 只在「轻」上赢，保暖与绒种都不对。

## 2026-09-03 四档方案补单价与单条重量
- 黑冰 E1000 L 1.40 kg；G1000 M/L 1.33/1.35 kg；B 与黑岩 D/暖阳、天石、挪客按吊牌或评测约值列出。

## 2026-09-03 四档睡袋重算
- 在原两档上增加：女鹅绒木乃伊+男鸭绒木乃伊，以及两人鸭绒木乃伊。
- 女方四档均按约 1000g；价格用两件套促销观察区间。

## 2026-09-03 鹅绒可拼信封方案
- 按 160/190、鹅绒、可拼、-10℃、出国轻量对比黑冰 E/G 与黑岩暖阳/木乃伊，并对照天石雪雁、挪客 CW。
- 最低价按两件套促销观察给出；强调同码拼接与 190 cm 长度卡边。

## 2026-09-03 睡袋品牌对比
- 核对黑冰、黑岩、探路者、探险者产品线、展开长度与约 700 g 羽绒档现价区间。
- 结论：同等羽绒条件下黑岩最便宜、黑冰 G 性价比更均衡；230 cm 只有探险者棉袋明确有货。

## 2026-09-03 自助换课需求理解与 grill
- 读群聊、武伶姝方案文档、CodeWiki、userauth `biz_exchange.go`
- 对照升学段转换，整理客服/产品待拍板问题与建议
- 索引复核队列为空；userauth 查询路径与索引基线一致

## 2026-09-03 五城对比排名
- 葫芦岛/青岛/大连/烟台/威海：口感、海鲜价、房价、拥挤分项 + 均权综合；写入飞书与本地 md。

## 2026-09-03 葫芦岛何时去
- 按往年海滨旺季、开海后价格节奏、疗养院 7–9 月工会床、2026 中秋国庆日历给出窗口建议。
- 飞书原文地插入章节「什么时候去性价比最高」，本地 md 同步。

## 2026-09-03 葫芦岛开海海鲜与疗养院
- 用户目标：9/1 开海后去葫芦岛吃海里捞上来、带去店里加工的海鲜；不去兴城古城；查老干部温泉疗养院如何预订。
- 官方确认 2026-09-01 12:00 黄渤海开海；葫芦岛点为兴城小坞渔港、绥中张监渔港。
- 海鲜：优先小坞渔港/码头加工 + 南关市场本地价；推荐海香园、敬友、钓鱼台渔家、龙湾小镇人家等。
- 疗养院：兴海南街 108–191 一带；OTA 上古泉度假酒店/水调歌头可订；交通/林业/电力多半电话订。
- 交付物改为飞书云文档（用户要求弃用 canvas，canvas 文件已删）：《葫芦岛开海：海鲜怎么吃、疗养院怎么订》https://guanghe.feishu.cn/docx/R4FpdP9YCouLFWxFe1EcR3z2nec ，位于生产力方舟 `Pu6TfLJIrlN0QIdbEM2cvUMMnFe`；本地源文件 `葫芦岛开海海鲜与疗养院.md`。上传前已做同目录重名检查，导入后 7 张表均为原生表格。
- 疗养院渠道交叉完成后补进：宝石花 OTA、望海楼身份限制、泉眼直供 vs 拉水、林疗工期、低价院不上平台。
- 海鲜口碑交叉完成后补进：开海第 3 天品种窗、邴家市场动线、觉华岛代加工、避开龙湾瞭望塔摊、注胶/注水辟谣。
