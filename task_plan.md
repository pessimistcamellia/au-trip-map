...
### 阶段 67：澳洲之旅租赁无人机选型
- [x] 核实大疆现役便携机型官方规格（重量、抗风、折叠尺寸、电池 Wh、影像规格）
- [x] 核实 CASA 娱乐飞行要求与本行程逐点禁飞／许可（WA DBCA、Tas PWS、Parks Victoria）
- [x] 核实租赁渠道（国内平台能否出境、澳洲当地租、二手买卖）与 13 天成本
- [x] 核实「晨曦Stanley」实际机型与旅拍博主主流配置
- [x] 结合 3 晚露营装备与两个 28 寸箱的容积重量约束给出推荐排序
- [x] 产出对比表并上传飞书「生产力方舟」
- **状态：** complete

### 阶段 67：中澳无人机租赁与买卖调研
- [x] 核实中国大陆主流租赁商品页、押金与平台责任条款
- [x] 重点检索设备出境限制和保险境外覆盖
- [x] 核实澳洲租赁商、游客证件、押金、门店与异地归还
- [x] 核实 DJI 官方价格、二手价证据与 13 天成本量级
- [x] 核实 CASA、WA/TAS/VIC 目的地飞行限制并形成结论
- **状态：** complete

### 阶段 65：智能客服 AI 自助换课评审包
- [x] 汇总用户确认口径，仅保留课程范围规则为待定
- [x] 制作整体流程图与 C 端高保真标注原型
- [x] 制作交互评审包并完成本地门禁
- [x] 发布 MinIO 稳定链接
- [x] 在生产力方舟创建需求说明与 SDD，并完成互链核验
- **状态：** complete

### 阶段 66：评审包与原型视觉重做（redesign-skill）
- [x] 按 redesign audit 诊断旧稿：紫+蓝双强调色、通用卡片、边框表格、无交互态
- [x] 评审包换暖纸底＋单一孔雀蓝强调色、编辑感排版、去边框表格、页签 hash 深链
- [x] 原型改为「手机列＋变更批注栏」标注布局，补 hover/active/focus 与动效
- [x] 流程图配色统一到新色板并重新出图
- [x] 修复 `DIFF_CHANGESET` 结构不匹配导致的批注栏失效
- [x] 发布 MinIO 并同步两份飞书文档的原型链接
- **状态：** complete

### 阶段 68：评审包迁到「新的评审包skill」的 GitLab Pages 通道
- [x] 比对两个评审包 skill：确认差异仅在发布通道，内容规范一致
- [x] 查明旧包 MinIO 写回 token 已于 9/14 过期，且网关改名导致原址续签不可行
- [x] 跑 `pull_user_edits.py` 闸门，确认线上无未回灌的人工编辑
- [x] `build.py` 换用新 skill 模板（GitLab 写回引擎）并改 `REVIEW_URL` / `PROTO_URL`
- [x] 修 `build.py` 默认参数为线上真源，避免裸跑丢原型 SDD 深链
- [x] 原型与评审包发布到 `pages.yc345.tv`，跑完交付门禁 19 项
- [x] 验证 Pages 匿名可访问与 GitLab Files API 跨域预检（保存发布可用）
- [x] 定点替换 SDD 与需求说明共 6 处链接，旧链残留 0
- **状态：** complete

### 阶段 67：澳大利亚无人机法规调研
- [x] 核实 CASA 娱乐飞行、标准操作条件、地图 App 与处罚
- [x] 核实西澳、塔州、维州公园及行程地点规则
- [x] 核实野生动物距离、入境申报与航司锂电池规定
- [x] 对无地点级官方结论的条目标注「未能核实」
- **状态：** complete
...
- [x] 地点详情「实用」页展示凭证卡片；点击全览、可关闭返回
- [x] 9/27 改为宜必思快捷珀斯机场；回填 4 家酒店确认号
- [x] 此前营地／船票／十二门徒截图一并挂附件
- [x] 测试、构建、发布
- **状态：** complete

### 阶段 69：回填已确认酒店／营地／船票预订
...
### 阶段 64：核实 220 cm 长度并找真 220 睡袋
- [x] 查实黑冰 L 码实际展开长（205，非 220），说明淘宝「加长型」字段的来源
- [x] 列出真 220 及以上：黑岩暖阳/和风加大、挪客 CW1000 L、三极 230、C2H3 215、Helsport Long
- **状态：** complete

### 阶段 63：迪卡侬睡袋能否对打四档方案
- [x] 核对中国官网类目与欧英站 FORCLAZ/SIMOND MT500/MT900、QUECHUA Arpenaz
- [x] 按重量、ISO 舒适/极限温标、单价对照四档（鹅信封 / 双鹅木乃伊 / 女鹅男鸭 / 双鸭）
- **状态：** complete

### 阶段 62：四档睡袋方案重算（含鸭绒木乃伊两档）
- [x] 分四档：鹅绒可拼信封 / 双鹅绒木乃伊 / 女鹅男鸭木乃伊 / 双鸭绒木乃伊
- [x] 重算黑冰、黑岩两件套最低价
- [x] 对照天石、挪客等同档更便宜方案
- **状态：** complete

### 阶段 61：鹅绒可拼接信封（160/190、-10℃、出国轻量）
- [x] 按鹅绒、可拼、10～-10℃、出国体积约束筛选黑冰/黑岩
- [x] 给出首选信封两件套与木乃伊兜底最低价区间
- [x] 对照天石、挪客等是否更划算
- **状态：** complete

### 阶段 60：黄渤海五城综合排名
- [x] 按口感、海鲜价、房价、拥挤度交叉公开数据
- [x] 给出分项排名与均权综合，写入飞书
- **状态：** complete

### 阶段 59：葫芦岛何时去性价比最高
- [x] 对照往年海滨旺季、开海后货价节奏、疗养院旺季
- [x] 叠 2026 中秋/国庆/补班日历给出窗口
- [x] 写入飞书文档与本地 md
- **状态：** complete

### 阶段 57：葫芦岛开海海鲜与疗养院预订
- [x] 检索开海后「海里捞、店里加工」的性价比海鲜去处（排除兴城古城）
- [x] 多源交叉核对口碑店、市场与渔家乐
- [x] 查清老干部/单位温泉疗养院名单与可预订渠道
- [x] 上传飞书「生产力方舟」
- **状态：** complete

### 阶段 56：删除冗余日级信息模块并重做地点视觉层级
- [x] 审计主路书、离线导出与地图／时间轴的地点类别呈现
- [x] 完整删除日程页“住宿与其他信息”折叠模块及离线导出同名冗余区块
- [x] 重新设计景点与住宿图标：景点高强调，住宿低调但仍可辨识
- [x] 补充／更新测试并通过 typecheck、Vitest、production build
- [x] 发布 gh-pages，使用移动端视口完成线上复验
- **状态：** completed

### 阶段 55：住宿图标全量审计与纠偏
- [x] 逐点读取 My Maps 实际图标 URL，查出 D9-5、D11-5 仍是默认蓝钉，D12-4 泰迪观景台被误套黑底白床
- [x] 定位根因：侧栏行在 `scrollIntoView` 动画期间漂移，旧脚本先测坐标后点击会打到相邻行
- [x] 新增 `/tmp/fix_icons.py`：等行坐标连续两次一致再测量、油漆桶只从目标行内取、改完立刻回读校验
- [x] 修正三点并刷新复验：8 个住宿点全部 `1602-hotel-bed` + `000000`，其余 63 点为默认蓝钉，图层顺序仍正确
- [x] 把 `build_kml.py` 的住宿口径与地图对齐（d9-05 纳入住宿图标，待下单状态由文字承载），重建 KML
- **状态：** complete
...
- [x] 用户确认后已同步飞书云文档与 My Maps 的 10/2 营地、10/3 改飞（本轮复核：文档含帐篷营位与朗塞斯顿改飞，无渡轮残留）
- **状态：** complete
...
- [x] 提交、发布 gh-pages 并线上复验（后续多轮发布已覆盖）
- **状态：** complete
- **遗留（不阻塞，未再被要求）：** 56 家餐厅缺评分，待需要时再补
